...
void Window::set_display_mode(DisplayMode mode)
{
    ...
    if (mode == FULLSCREEN)
    {
        ...
        glfwSetWindowMonitor(
            m_window, nullptr, m_primary_monitor->x, m_primary_monitor->y, m_primary_monitor->width,
            m_primary_monitor->height, 0);
...
{
    static_cast<Window *>(glfwGetWindowUserPointer(window))->dispatch(WindowResized, WindowResizeEvent(width, height));
}
...
void Window::on_debug()
{
    if (ImGui::TreeNode("Window"))
    {
        ...
        if (ImGui::Combo(
                ...
                &m_current_monitor->resolutions, m_current_monitor->resolutions.size()))
        {
            set_resolution(
