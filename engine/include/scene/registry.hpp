...
class Registry
{
  ...
  public:
    ...
    {
        ComponentStorage<T> *storage = pool.get<T>();
        if (storage->search(entity) == SSet::null)
        {
            return nullptr;
        }
    ...
    bool any_of(uint32_t const entity)
    {
        ...
        SSet &entities = static_cast<SSet &>(*(pool.get<std::tuple_element_t<0, std::tuple<Ts...>>>()));

        // perform a union on all following sets
        ((const_cast<SSet &>(entities) = const_cast<SSet &>(entities).join(static_cast<SSet &>(*pool.get<Ts>()))), ...);

        return entities.search(entity) != SSet::null;
    }
    ...
    bool all_of(uint32_t const entity)
    {
        ...
        ((const_cast<SSet &>(entities) = const_cast<SSet &>(entities).intersect(static_cast<SSet &>(*pool.get<Ts>()))),
         ...);
